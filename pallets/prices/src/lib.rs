...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        type Source: DataProvider<AssetIdentifier, TimeStampedPrice>
            + DataProviderExtended<AssetIdentifier, TimeStampedPrice>;
        ...
        type LiquidStakingCurrenciesProvider: LiquidStakingCurrenciesProvider<AssetIdentifier>;
    ...
    pub enum Event<T: Config> {
        ...
        SetPrice(AssetIdentifier, Price),
        /// Reset emergency price. \[asset_id\]
        ResetPrice(AssetIdentifier),
    }
    ...
    pub type EmergencyPrice<T: Config> =
        StorageMap<_, Twox64Concat, AssetIdentifier, Price, OptionQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_price(
            ...
            <Pallet<T> as EmergencyPriceFeeder<AssetIdentifier, Price>>::set_emergency_price(
                asset_id, price,
            );
            Ok(().into())
        }
        ...
        pub fn reset_price(
            ...
            T::FeederOrigin::ensure_origin(origin)?;
            <Pallet<T> as EmergencyPriceFeeder<AssetIdentifier, Price>>::reset_emergency_price(
                asset_id,
            );
            Ok(().into())
        }
...
impl<T: Config> Pallet<T> {
    ...
    fn get_emergency_price(asset_id: &AssetIdentifier) -> Option<PriceDetail> {
...
impl<T: Config> EmergencyPriceFeeder<AssetIdentifier, Price> for Pallet<T> {
    /// Set emergency price
...
impl<T: Config> DataProviderExtended<AssetIdentifier, TimeStampedPrice> for Pallet<T> {
    fn get_no_op(asset_id: &AssetIdentifier) -> Option<TimeStampedPrice> {
        match T::LiquidStakingCurrenciesProvider::get_staking_currency()
            .zip(T::LiquidStakingCurrenciesProvider::get_liquid_currency())
        {
